...
with DAG(
    ...
    def download_data_from_gcs(**kwargs):
        ...
        bucket_name = "uber_data_engineering_project_166"
    ...
    def transform_data(**kwargs):
        ...
        print(f"datetime_dim size is {len(datetime_dim)}")

        passenger_count_dim = df[['passenger_count']].drop_duplicates().reset_index(drop=True)
        passenger_count_dim['passenger_count_id'] = passenger_count_dim.index
        passenger_count_dim = passenger_count_dim[['passenger_count_id','passenger_count']]
        print(f"passenger_count_dim size is {len(passenger_count_dim)}")
        ...
        print(f"trip_distance_dim size is {len(trip_distance_dim)}")

        rate_code_dim = df[['RatecodeID']].drop_duplicates().reset_index(drop=True)
        rate_code_dim['rate_code_id'] = rate_code_dim.index
        rate_code_dim['rate_code_name'] = rate_code_dim['RatecodeID'].map(rate_code_type)
        rate_code_dim = rate_code_dim[['rate_code_id','RatecodeID','rate_code_name']]
        print(f"rate_code_dim size is {len(rate_code_dim)}")

        pickup_location_dim = df[['pickup_longitude', 'pickup_latitude']].drop_duplicates().reset_index(drop=True)
        pickup_location_dim['pickup_location_id'] = pickup_location_dim.index
        pickup_location_dim = pickup_location_dim[['pickup_location_id','pickup_latitude','pickup_longitude']] 
        print(f"pickup_location_dim size is {len(pickup_location_dim)}")

        dropoff_location_dim = df[['dropoff_longitude', 'dropoff_latitude']].drop_duplicates().reset_index(drop=True)
        dropoff_location_dim['dropoff_location_id'] = dropoff_location_dim.index
        dropoff_location_dim = dropoff_location_dim[['dropoff_location_id','dropoff_latitude','dropoff_longitude']]
        print(f"dropoff_location_dim size is {len(dropoff_location_dim)}")
        ...
        print(f"payment_type_dim size is {len(payment_type_dim)}")
        ...
        print(f"fact_table size is {len(fact_table)}")
    ...
    def load_to_bigquery(**kwargs):
        ...
        for table_name, file_path in transformed_file_paths.items():
            table_id = f"careful-analyst-451322-v9.uber_dataset.{table_name}"  # Replace with your project and dataset
